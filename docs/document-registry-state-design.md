...
3b. **"读到"不等于"读全"**：OpenRAG 的 `files/get_all` 最多返回 500 条且不告知是否截断，
   而对账是拿名单做**集合差**的 —— 被截掉的名字会统统变成"登记了但远端没有"，一次分页
   截断就是一屏假告警（而这条命令正是给 cron 用的）。所以端口契约写成
   **"要么全量、要么报错，绝不返回部分列表"**，适配器在命中 `LIST_FILES_MAX` 时抛错，
   该租户照旧进 `remote_errors`。这是"读不到 ≠ 没有"的加强版：**读到了 ≠ 读全了**。

4. **多一类"矛盾行"**：`remote_outcome_unknown` 只允许出现在 `FAILED`/`DELETED` 上
   （那两个状态才意味着"没有定论就定了终态"）。它若出现在别的状态上（如 `INDEXED`），
   报告按 `inconsistent` 报出来，且**优先于年龄门**——否则"可能还在途"会成为矛盾行的藏身处。
   这一格当前写入路径到不了（`mark_indexed` 会清标记），但"到不了"是关于现在的断言：
   报告的意义就是两边对不上就说出来，而不是替将来坏掉的写路径打掩护。
   查询（`list_unsettled`）是候选**超集**、分类才是判定，两者的耦合由一条遍历
   整个"状态 × 标记"空间的测试钉住。
