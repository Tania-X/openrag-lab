...
### 评审第 6 轮抓到的 4 级 bug：`--tenant ""` 被短路成全量遍历

CLI 用 `[tenant_slug] if tenant_slug else None` 构造过滤：空字符串 falsy → 变成 `None`
→ 走"遍历全部租户"分支。于是**服务层那条"空过滤器拒绝"守卫（还带着测试）从 CLI 路径上
根本不可达** —— 用户以为在查某个租户，实际拿到的是全量报告。

修法一行：`None if tenant_slug is None else [tenant_slug]`（区分"没提供"与"提供了空值"）。

**这条的意义在测试层面**：评审明确指出 `test_an_empty_filter_is_refused` "绕过了 CLI 的
短路，因此挡不住这条路径"。这就是"只测了单元、没测接线"的空转陷阱 ——
**一个守卫有测试，不等于它在真实调用路径上可达**。现在两条路径各有一条测试
（服务层拒绝空过滤、CLI 拒绝 `--tenant ""`）。

**三个 4 级 bug 有一个共同形状**：switch/短路/过滤这些"决定要不要看"的地方写错了，
而它们**不改变功能、只改变信号的范围**（一个把打错的 slug 变成绿灯，一个把截断变成
一屏假 missing，一个把空值变成全量）。给告警系统写工具时，先审"我到底看了什么"。
...
于是删掉防御代码，改成钉住那条性质本身（命名空间必须分隔符结尾）。

### 存活变异体的三种，而不是两种（P3 补）

1. **真漏测** → 补测试；
2. **这段代码本来就没用**（等价于「防御了不可能发生的事」）→ 删代码，不是补测试；
3. **结构改进、无可观测行为差异**（P3 第 5 次推送遇到）→ 测试在原则上抓不到它，
   只能靠代码结构本身表达意图。承认这一点，别假装有测试覆盖。

第 3 类的实例：把 `render_report(report)` 从 `finally` 之后挪进 `try` 块内（消除"必须依赖
...
### 补记：`reset_db` 必须真的断开连接（评审第 1 轮之后）

评审指出 `str(URL)` 会把密码渲染成 `***`，于是"换库守卫"在"只差密码"时被绕过。实证成立
（SQLAlchemy 2.0.52：`str(p1) == str(p2)` 为 True，而 `p1 == p2` 为 False），修法是直接比较
URL 对象（`URL.__eq__` 含密码）。

**修的过程中顺带被 pytest 的告警抓出第二个问题**：`reset_db()` 只丢引用、不断开连接，于是
aiosqlite 的连接在事件循环结束后被 GC，套件里出现一条 `PytestUnraisableExceptionWarning`。
生产代码没这个问题（API 在 lifespan 的 `finally` 里 `await engine.dispose()`），但"解绑"这个
动作本身不该留下半个连接 —— 现在 `reset_db()` 是 async 且真的 dispose，API 与测试共用它。

**告警一多，真问题就淹了**：这条告警本来完全无害，但它是被"跑一遍 -q 时多了一行 warning"抓到的
—— 如果当时放过，下一个真告警也会长这样。
